...
import Checkbox from "../4-form/Checkbox";
...
export default function LoginForm({
  ...
  demoCredentials = [
    { role: "Staff", email: "staff@blackbird.com", pass: "staff123" },
    { role: "Manager", email: "manager@blackbird.com", pass: "manager123" },
  ],
}) {
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      {showDemoCredentials && demoCredentials && demoCredentials.length > 0 && (
        <div className="mt-8 rounded-2xl border border-slate-100 bg-[#FAF6F0] p-4 text-sm text-slate-700">
          <p className="font-semibold text-slate-900">Demo credentials:</p>
          {demoCredentials.map((cred, i) => (
            <p key={i} className="mt-1.5 text-xs text-[#5F3A27] font-medium">
              {cred.role}: <span className="text-slate-600 font-normal">{cred.email}</span> / <span className="text-slate-600 font-normal">{cred.pass}</span>
            </p>
          ))}
        </div>
      )}
